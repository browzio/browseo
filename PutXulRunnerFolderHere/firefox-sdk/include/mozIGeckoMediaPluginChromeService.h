...
class NS_NO_VTABLE mozIGeckoMediaPluginChromeService : public nsISupports {
 public:
  ...
  NS_IMETHOD ForgetThisSite(const nsAString & site, const nsAString & aPattern) = 0;
...
#define NS_DECL_NON_VIRTUAL_MOZIGECKOMEDIAPLUGINCHROMESERVICE \
  ...
  NS_METHOD ForgetThisSite(const nsAString & site, const nsAString & aPattern); \
...
/* void forgetThisSite (in AString site, in DOMString aPattern); */
NS_IMETHODIMP _MYCLASS_::ForgetThisSite(const nsAString & site, const nsAString & aPattern)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
