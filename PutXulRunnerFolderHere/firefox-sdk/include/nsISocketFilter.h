...
/* Header file */
class nsSocketFilter : public nsISocketFilter
{
public:
  NS_DECL_ISUPPORTS
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsSocketFilter, nsISocketFilter)

nsSocketFilter::nsSocketFilter()
{
...
/* bool filterPacket ([const] in NetAddrPtr remote_addr, [array, size_is (len), const] in uint8_t data, in unsigned long len, in long direction); */
NS_IMETHODIMP nsSocketFilter::FilterPacket(const mozilla::net::NetAddr *remote_addr, const uint8_t *data, uint32_t len, int32_t direction, bool *_retval)
{
...
#define NS_DECL_NON_VIRTUAL_NSISOCKETFILTERHANDLER \
  NS_METHOD NewFilter(nsISocketFilter * *_retval); 
...
/* Header file */
class nsSocketFilterHandler : public nsISocketFilterHandler
{
public:
  NS_DECL_ISUPPORTS
  NS_DECL_NSISOCKETFILTERHANDLER

  nsSocketFilterHandler();
...
/* Implementation file */
NS_IMPL_ISUPPORTS(nsSocketFilterHandler, nsISocketFilterHandler)

nsSocketFilterHandler::nsSocketFilterHandler()
{
  /* member initializers and constructor code */
}
...
/* nsISocketFilter newFilter (); */
NS_IMETHODIMP nsSocketFilterHandler::NewFilter(nsISocketFilter * *_retval)
{
    return NS_ERROR_NOT_IMPLEMENTED;
}
