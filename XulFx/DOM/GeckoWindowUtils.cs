...
namespace Gecko.DOM
{
	public sealed class GeckoWindowUtils : ComObject<nsIDOMWindowUtils>, IGeckoObjectWrapper
	{
		public static GeckoWindowUtils Create(nsIDOMWindowUtils windowUtils)
		{
			return new GeckoWindowUtils(windowUtils);
		}

		private GeckoWindowUtils(nsIDOMWindowUtils windowUtils)
			: base(windowUtils)
		{
		...
		{
			get { return Instance.GetImageAnimationModeAttribute(); }
			set { Instance.SetImageAnimationModeAttribute(value); }
		}
		...
		{
			get { return Instance.GetDocCharsetIsForcedAttribute(); }
		}
		...
		{
			return nsString.Get(Instance.GetDocumentMetadata, name);
			//using (nsAString tempName = new nsAString(name), ret = new nsAString())
			//{
			//	Instance.GetDocumentMetadata( tempName, ret );
			//	return ret.ToString();
			//}
		}
		...
		{
			Instance.SetDisplayPortForElement(aXPx, aYPx, aWidthPx, aHeightPx, aElement.Instance, aPriority);
		}
		...
		{
			Instance.SetResolution(aResolution);
		}
		...
		/// during dispatch
		/// </summary>		
		public void SendMouseEvent(string aType, float aX, float aY, GeckoMouseButton aButton, int aClickCount, int aModifiers, bool aIgnoreRootScrollFrame, float aPressure, ushort aInputSourceArg, bool isDOMEventSynthesized, bool isWidgetEventSynthesized, int buttons)
		{
			using (nsAString type = new nsAString(aType))
			{
				Instance.SendMouseEvent(type, aX, aY, (int)aButton, aClickCount, aModifiers, aIgnoreRootScrollFrame, aPressure, aInputSourceArg, isDOMEventSynthesized, isWidgetEventSynthesized, buttons, 12);
			}
		}
		...
		/// </summary>		
		public void SendMouseEventToWindow(string aType, float aX, float aY, GeckoMouseButton aButton, int aClickCount, int aModifiers, bool aIgnoreRootScrollFrame, float aPressure, ushort aInputSourceArg, bool isDOMEventSynthesized, bool isWidgetEventSynthesized, int buttons)
		{
			using (nsAString type = new nsAString(aType))
			{
				Instance.SendMouseEventToWindow(type, aX, aY, (int)aButton, aClickCount, aModifiers, aIgnoreRootScrollFrame, aPressure, aInputSourceArg, isDOMEventSynthesized, isWidgetEventSynthesized, buttons, 12);
			}
		}
		...
		{
			Instance.SendWheelEvent(aX, aY, aDeltaX, aDeltaY, aDeltaZ, aDeltaMode, aModifiers, aLineOrPageDeltaX, aLineOrPageDeltaY, aOptions);
		}
		...
		public bool SendKeyEvent(string aType, int aKeyCode, int aCharCode, int aModifiers, bool aPreventDefault)
		{
			using (nsAString type = new nsAString(aType))
			{
				return Instance.SendKeyEvent(type, aKeyCode, aCharCode, aModifiers, aPreventDefault ? 1U : 0U);
			}
		}
		...
		/// privileges.
		/// </summary>		
		public void SendNativeKeyEvent(int aNativeKeyboardLayout, int aNativeKeyCode, int aModifierFlags, string aCharacters, string aUnmodifiedCharacters, nsIObserver observer)
		{
			using (nsAString characters = new nsAString(aCharacters), unmodifiedCharaters = new nsAString(aUnmodifiedCharacters))
			{
				Instance.SendNativeKeyEvent(aNativeKeyboardLayout, aNativeKeyCode, aModifierFlags, characters, unmodifiedCharaters, observer);
			}
		}
		...
		/// privileges.
		/// </summary>
		public void SendNativeMouseEvent(int aScreenX, int aScreenY, int aNativeMessage, int aModifierFlags, GeckoElement aElement, nsIObserver observer)
		{
			Instance.SendNativeMouseEvent(aScreenX, aScreenY, aNativeMessage, aModifierFlags, aElement.Instance, observer);
		}
		...
		{
			nsString.Set(Instance.ActivateNativeMenuItemAt, indexString);
		}
		...
		{
			nsString.Set(Instance.ForceUpdateNativeMenuAt, indexString);
		}
		...
		{
			Instance.Focus(aElement.Instance);
		}
		...
		{
			Instance.GarbageCollect(aListener, aExtraForgetSkippableCalls);
		}
		...
		{
			return Instance.ElementFromPoint(aX, aY, aIgnoreRootScrollFrame, aFlushLayout).Wrap(GeckoElement.Create);
		}
		...
		{
			return Instance.NodesFromRect(aX, aY, aTopSize, aRightSize, aLeftSize, aBottomSize, aIgnoreRootScrollFrame, aFlushLayout).Wrap(GeckoNodeCollection.Create);
		}
		...
		/// fired.
		/// </summary>				
		public bool IsMozAfterPaintPending
		{
			get
			{
				return Instance.GetIsMozAfterPaintPendingAttribute();
			}
		}
		...
		{
			Instance.SuppressEventHandling(aSuppress);
		}

		public void ClearMozAfterPaintEvents()
		{
			Instance.ClearMozAfterPaintEvents();
		}
		...
		/// privileges.
		///
		/// @param disable  If true, disable all non synthetic test mouse events
		/// on all windows.  Otherwise, enable them.
		/// </summary>
		public void DisableNonTestMouseEvents(bool disable)
		{
			Instance.DisableNonTestMouseEvents(disable);
		}
		...
		/// </summary>
		/// <param name="flushLayout">flushes layout if true. Otherwise, no flush occurs.</param>
		public void GetScrollXY(bool flushLayout, out int scrollX, out int scrollY)
		{
			Instance.GetScrollXY(flushLayout, out scrollX, out scrollY);
		}
		...
		{
			get { return Instance.GetIMEIsOpenAttribute(); }
		}
		...
		{
			get { return Instance.GetIMEStatusAttribute(); }
		}
		...
		{
			get { return Instance.GetScreenPixelsPerCSSPixelAttribute(); }
		}
		...
		/// </summary>		
		public bool DispatchDOMEventViaPresShell(GeckoNode target, GeckoEvent @event, bool trusted)
		{
			if (target == null)
				throw new ArgumentNullException("target");
			if (@event == null)
				throw new ArgumentNullException("event");
			return Instance.DispatchDOMEventViaPresShell(target.Instance, @event.Instance, trusted);
		}
		...
		public void SendTextEvent(string aCompositionString, int aFirstClauseLength, uint aFirstClauseAttr,
			int aSecondClauseLength, uint aSecondClauseAttr, int aThirdClauseLength, uint aThirdClauseAttr, int aCaretStart, int aCaretLength)
		{
			throw new NotImplementedException();
		}
		...
		/// </summary>		
		public ulong OuterWindowID
		{
			get
			{
				return Instance.GetOuterWindowIDAttribute();
			}
		}
		...
		/// </summary>		
		public ulong CurrentInnerWindowID
		{
			get
			{
				return Instance.GetCurrentInnerWindowIDAttribute();
			}
		}
		...
		{
			return Instance.IsInModalState();
		}
		...
		{
			Instance.SuspendTimeouts();
		}

		public void ResumeTimeouts()
		{
			Instance.ResumeTimeouts();
		}
		...
		{
			get { return Instance.GetDisplayDPIAttribute(); }
		}
		...
		{
			return Instance.GetOuterWindowWithId(aOuterWindowID).Wrap(GeckoWindow.Create);
		}

		public void RenderDocument(IntPtr aRect, uint aFlags, nscolor aBackgroundColor, IntPtr aThebesContext)
		{
			throw new NotImplementedException();
		}
		...
		{
			get { return Instance.GetFocusedInputTypeAttribute(); }
		}
		...
		/// otherwise.
		/// </summary>
		public bool PaintingSuppressed
		{
			get { return Instance.GetPaintingSuppressedAttribute(); }
		}
		...
		/// </param>
		public void GetScrollbarSize(bool flushLayout, out int width, out int height)
		{
			Instance.GetScrollbarSize(flushLayout, out width, out height);
		}
	}
}
