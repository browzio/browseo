...
class NS_NO_VTABLE nsITooltipListener : public nsISupports {
 public:
  ...
  NS_IMETHOD OnShowTooltip(int32_t aXCoords, int32_t aYCoords, const char16_t * aTipText, const char16_t * aTipDir) = 0;
...
#define NS_DECL_NSITOOLTIPLISTENER \
  NS_IMETHOD OnShowTooltip(int32_t aXCoords, int32_t aYCoords, const char16_t * aTipText, const char16_t * aTipDir) override; \
  NS_IMETHOD OnHideTooltip(void) override; 

/* Use this macro when declaring the members of this interface when the
   class doesn't implement the interface. This is useful for forwarding. */
#define NS_DECL_NON_VIRTUAL_NSITOOLTIPLISTENER \
  NS_METHOD OnShowTooltip(int32_t aXCoords, int32_t aYCoords, const char16_t * aTipText, const char16_t * aTipDir); \
  NS_METHOD OnHideTooltip(void); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSITOOLTIPLISTENER(_to) \
  NS_IMETHOD OnShowTooltip(int32_t aXCoords, int32_t aYCoords, const char16_t * aTipText, const char16_t * aTipDir) override { return _to OnShowTooltip(aXCoords, aYCoords, aTipText, aTipDir); } \
...
#define NS_FORWARD_SAFE_NSITOOLTIPLISTENER(_to) \
  NS_IMETHOD OnShowTooltip(int32_t aXCoords, int32_t aYCoords, const char16_t * aTipText, const char16_t * aTipDir) override { return !_to ? NS_ERROR_NULL_POINTER : _to->OnShowTooltip(aXCoords, aYCoords, aTipText, aTipDir); } \
...
/* void onShowTooltip (in long aXCoords, in long aYCoords, in wstring aTipText, in wstring aTipDir); */
NS_IMETHODIMP nsTooltipListener::OnShowTooltip(int32_t aXCoords, int32_t aYCoords, const char16_t * aTipText, const char16_t * aTipDir)
{
